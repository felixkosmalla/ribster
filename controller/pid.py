...
def writeTemp(_t):
    # write to db
    tInstance = dbs.Event.create(param_3 = _t, event_type = dbs.Event.PID_TEMPERATURE)
    tInstance.save()


def writeHeat(_t):
    # write to db
    tInstance = dbs.Event.create(param_3 = _t, event_type = dbs.Event.PID_HEATING)
    tInstance.save()
...
if __name__=="__main__":
    ...
    while True:
        ...
        time_on = sampleTime * (duty_cycle / 100.0)
        time_off =  sampleTime * (1.0  -  (duty_cycle / 100.0))
